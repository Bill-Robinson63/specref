...
class SpecRef(SpecRefTemplate):
  def __init__(self, **properties):
    ...
    # Let's start off with Hα, Hβ, Hγ, Hδ, HeII, NV, O_III, O_I, S_II, N_II which are said to be most common/prominent for ameture astronomers
    self.spectralLines = {}
  ...
  def canvas_1_reset(self, **event_args):
    ...
    if self.H_gamma.checked:
      SpecRef.displayLine(self, "Hγ")
    if self.H_delta.checked:
      SpecRef.displayLine(self, "Hδ")
    if self.He_II.checked:
      SpecRef.displayLine(self, "HeII")
    if self.N_V.checked:
      SpecRef.displayLine(self, "NV")
    if self.O_III.checked:
      SpecRef.displayLine(self, "OIII")
    if self.S_II.checked:
      SpecRef.displayLine(self, "SII")

  @handle("emission", "clicked")
  def emission_clicked(self, **event_args):
    self.canvas_1_reset()

  @handle("absorption", "clicked")
  def absorption_clicked(self, **event_args):
    self.canvas_1_reset()

  @handle("frequency", "clicked")
  def frequency_clicked(self, **event_args):
    self.canvas_1_reset()

  @handle("wavelength", "clicked")
  def wavelength_clicked(self, **event_args):
    self.canvas_1_reset()

  @handle("H_alpha", "change")
  def H_alpha_change(self, **event_args):
    self.canvas_1_reset()

  @handle("H_beta", "change")
  def H_beta_change(self, **event_args):
    self.canvas_1_reset()

  @handle("H_gamma", "change")
  def H_gamma_change(self, **event_args):
    self.canvas_1_reset()

  @handle("H_delta", "change")
